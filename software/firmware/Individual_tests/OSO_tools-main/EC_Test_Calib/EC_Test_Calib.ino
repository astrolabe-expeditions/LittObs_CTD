/* Code for reading and sending command to EZO-RTD from Atlas Scientific
Use for : 
- Read temperature,
- Test EZO_RTD device
- Calibration
...
//const int ledpin = XX; 
const int startpin = 4; //4; ou 27 pour SO
...
void loop() {                                                                     //the main loop.
  ...
  if (serial_event == true) {                                                     //if a command was sent to the EZO device.
    ...
    int i = 0;
    while (computerdata[i] != 0) Wire.write((uint8_t)computerdata[i++]);
